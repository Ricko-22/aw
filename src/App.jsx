...
function App() {
  ...
  // Tracking states
  const [trackingMode, setTrackingMode] = useState("invoice");
  ...
  const getInputType = () => trackingMode === "id" ? "number" : "text";
    ...
    <div className="app">
      ...
      <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
        <div className="navbar-content">
          ...
          <button className={`hamburger ${mobileMenuOpen ? 'active' : ''}`} onClick={(e) => { e.stopPropagation(); setMobileMenuOpen(!mobileMenuOpen); }} aria-label="Toggle menu">
          ...
          <div className="nav-contact"><span className="status-dot"></span>ONLINE</div>
        </div>
      </nav>
      ...
      <header id="home" className={`hero ${imgLoaded ? 'hero-loaded' : 'hero-loading'}`} style={imgLoaded ? { backgroundImage: `url(/src/assets/bg1.png)` } : {}}>
        ...
        <div className="hero-content">
          ...
          <div className="hero-buttons">
            <button className="btn-main" onClick={() => scrollToSection('layanan')}>Pesan Sekarang</button>
            <button className="btn-secondary" onClick={() => scrollToSection('tracking')}>📦 Lacak Pesanan</button>
          </div>
        </div>
      </header>
      ...
      <section id="about" className="about-sec">
        <div className="about-content">
          <div className="about-text">
            ...
            <p className="about-desc">AW Laundry adalah layanan laundry profesional di Dukuhwaluh, Purwokerto yang mengutamakan kebersihan, kecepatan, dan kenyamanan pelanggan. Kami hadir untuk memudahkan hidup Anda!</p>
            <div className="about-features">
              <div className="about-feature-item"><div className="feature-icon-box">🧴</div><div><h5>Deterjen Premium</h5><p>Menggunakan deterjen berkualitas tinggi yang aman untuk semua jenis kain</p></div></div>
              <div className="about-feature-item"><div className="feature-icon-box">⏰</div><div><h5>Tepat Waktu</h5><p>Kami berkomitmen menyelesaikan cucian sesuai jadwal yang dijanjikan</p></div></div>
              <div className="about-feature-item"><div className="feature-icon-box">🚗</div><div><h5>Antar Jemput Gratis</h5><p>Layanan antar jemput gratis untuk area Dukuhwaluh & sekitarnya</p></div></div>
            </div>
          </div>
          <div className="about-stats">
            <div className="stat-card"><span className="stat-number">500+</span><span className="stat-label">Pelanggan Puas</span></div>
            <div className="stat-card"><span className="stat-number">3+</span><span className="stat-label">Tahun Pengalaman</span></div>
            <div className="stat-card"><span className="stat-number">6</span><span className="stat-label">Jenis Layanan</span></div>
            <div className="stat-card"><span className="stat-number">24/7</span><span className="stat-label">Order Online</span></div>
          </div>
        </div>
      </section>
      ...
      <section id="layanan">
        ...
        <div className="grid">
          {layananList.map((item) => (
            <div key={item.nama} className="card-service" onClick={() => { setLayanan(item.nama); setShowForm(true); }}>
      ...
      <section id="harga" className="harga-sec">
        ...
        <div className="harga-table-wrapper">
          <table className="harga-table">
            <thead><tr><th>Layanan</th><th>Harga</th><th>Estimasi</th><th>Keterangan</th></tr></thead>
            <tbody>
              <tr><td><span className="table-icon">👕</span> Cuci Setrika</td><td className="table-price">Rp 7.000/kg</td><td>2-3 Hari</td><td>Cuci + setrika rapi</td></tr>
              <tr className="table-highlight"><td><span className="table-icon">⚡</span> Laundry Express</td><td className="table-price">Rp 12.000/kg</td><td>6 Jam</td><td>Prioritas utama ⭐</td></tr>
              <tr><td><span className="table-icon">👟</span> Cuci Sepatu</td><td className="table-price">Rp 25.000/psg</td><td>2-3 Hari</td><td>Deep clean + deodorize</td></tr>
              <tr><td><span className="table-icon">✨</span> Cuci Selimut</td><td className="table-price">Rp 20.000/pcs</td><td>2-3 Hari</td><td>Termasuk pewangi</td></tr>
              <tr><td><span className="table-icon">🛏️</span> Cuci Sprei</td><td className="table-price">Rp 15.000/pcs</td><td>2-3 Hari</td><td>Bersih & harum</td></tr>
              <tr><td><span className="table-icon">🧼</span> Cuci Karpet</td><td className="table-price">Rp 30.000/m²</td><td>3-5 Hari</td><td>Vacuum + cuci basah</td></tr>
            </tbody>
          </table>
        </div>
      ...
      <section id="cara-kerja">
        ...
        <div className="steps-container">
          {[
            { num: 1, title: "Order Online", desc: "Hubungi kami via WhatsApp atau Web." },
            { num: 2, title: "Kami Jemput", desc: "Kurir akan menjemput cucian ke lokasimu." },
            { num: 3, title: "Proses Cuci", desc: "Pakaian dicuci higienis & disetrika rapi." },
            { num: 4, title: "Antar Kembali", desc: "Cucian wangi siap antar ke depan pintu." },
          ].map((step) => (
            <div key={step.num} className="step-item">
              <div className="step-num">{step.num}</div>
              <div><h5 style={{ fontWeight: '700' }}>{step.title}</h5><p style={{ fontSize: '13px', color: '#64748b' }}>{step.desc}</p></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="tracking" className="tracking-sec">
        ...
        <div className="tracking-card">
          <div className="tracking-tabs">
            <button className={`tracking-tab ${trackingMode === 'invoice' ? 'active' : ''}`} onClick={() => { setTrackingMode('invoice'); setTrackingInput(''); resetTracking(); }}>
              <span className="tab-icon">🧾</span><span className="tab-text">Kode Invoice</span>
            </button>
            <button className={`tracking-tab ${trackingMode === 'hp' ? 'active' : ''}`} onClick={() => { setTrackingMode('hp'); setTrackingInput(''); resetTracking(); }}>
              <span className="tab-icon">📱</span><span className="tab-text">Nomor HP</span>
            </button>
            <button className={`tracking-tab ${trackingMode === 'id' ? 'active' : ''}`} onClick={() => { setTrackingMode('id'); setTrackingInput(''); resetTracking(); }}>
              <span className="tab-icon">🔢</span><span className="tab-text">ID Transaksi</span>
            </button>
          </div>
          <div className="tracking-input-area">
            ...
            <div className="input-group">
              <input type={getInputType()} placeholder={getPlaceholder()} value={trackingInput} onChange={(e) => setTrackingInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleTracking()} />
              <button className="btn-cek" onClick={handleTracking} disabled={trackingLoading}>
                {trackingLoading ? <span className="btn-loading">⏳ Mencari...</span> : 'Cek Status'}
              </button>
            </div>
          </div>
          {trackingResult && (
      ...
      <section id="contact" className="contact-sec">
        ...
        <div className="contact-cards">
          <div className="contact-card">
            ...
            <a href="https://maps.app.goo.gl/tK8G8Tdb3qiKaZHo9?g_st=aw" target="_blank" rel="noopener noreferrer" className="contact-link">Lihat di Maps →</a>
          </div>
          <div className="contact-card">
            <div className="contact-icon-box">💬</div>
            <h4>WhatsApp</h4>
      ...
      {showForm && (
        <Suspense fallback={
          <div className="modal-overlay">
            <div className="modal-box" style={{ textAlign: 'center', padding: '40px' }}><p>⏳ Memuat form...</p></div>
          </div>
        }>
          <ModalPesanan layanan={layanan} setLayanan={setLayanan} nama={nama} setNama={setNama} hp={hp} setHp={setHp} alamat={alamat} setAlamat={setAlamat} onKirim={kirimPesanan} onTutup={() => setShowForm(false)} />
        </Suspense>
      )}
